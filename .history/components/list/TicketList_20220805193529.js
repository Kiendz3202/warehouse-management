...
function TicketList() {
    ...
    const ticketSupplier = useSelector((state) => state.ticket.tikcet)
    ...
    useEffect(() => {
        const fetchTicket = async () => {
            try {
                ...
                setPagination(prev => ({
                    ...prev,
                    pageCount: !isSearchingSelector ? Math.ceil(data.data.length / prev.numberPerPage) : Math.ceil(ticketSupplier.length / prev.numberPerPage)
                }))

                const ticketDisplay = !isSearchingSelector ? data.data.slice(pagination.offset, parseInt(pagination.offset) + parseInt(pagination.numberPerPage)) : ticketSupplier.slice(pagination.offset, parseInt(pagination.offset) + parseInt(pagination.numberPerPage))
