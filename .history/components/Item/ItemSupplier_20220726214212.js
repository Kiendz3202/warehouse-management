...
function ItemSupplier({ id, name, phone, email }) {
    ...
    return (
        <div className=' grid grid-cols-12 gap-3 text-black  border-black border-t'>
            ...
            <Link href={`/products-management/supplier/${id}`}><div className=' col-span-3 my-auto hover:border-b border-black cursor-pointer  truncate'>{name}</div></Link>
            <div className=' col-span-5 my-auto truncate'>{email}</div>
