...
function AddUser() {
    ...
    const addUserHandle = async () => {
        ...
        const token = localStorage.getItem('token')
        try {
            const res = await fetch('https://scm-tool.thanhpp.ninja/user', {
                method: 'POST',
                body: JSON.stringify({
                    name,
                    username,
            ...
            <div className='my-[32px]'>
                <div>
                    ...
                    <div className='flex mb-[26px]'>
                        ...
                        <input id='description' type='text' onChange={usernameHandle} className='text-black h-[38px] w-[380px] pl-[15px] pr-[40px] truncate' />
                    </div>
                    <div className='flex mb-[26px]'>
                        <div className='flex justify-end w-[175px] leading-[38px] mr-[20px]'>
                            <label htmlFor='location'>Location<span className='text-[#d13143]'>*</span></label>
                        </div>
                        <input id='location' type='text' onChange={passwordHandle} className='text-black h-[38px] w-[380px] pl-[15px] pr-[40px] truncate' />
                    </div>
                    <div className='flex mb-[26px]'>
